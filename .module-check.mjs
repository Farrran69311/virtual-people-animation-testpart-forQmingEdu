      ...
      function speakWithBrowserVoice(text) {
        ...
        try {
          ...
          utterance.onstart = () => {
            speechMeta.textContent = `纯前端口型测试 · 浏览器语音输出中${preferred ? ` · ${preferred.name}` : ''}`;
          };

          utterance.onerror = () => {
            speechMeta.textContent = '纯前端口型测试 · 浏览器语音输出失败，已继续保留无声口型';
          };

          utterance.onend = () => {
            if (!state.isSpeaking) {
              speechMeta.textContent = `纯前端口型测试 · 已完成${preferred ? ` · ${preferred.name}` : ''}`;
            }
          };
